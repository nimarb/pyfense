...
class PyFenseEntities(cocos.layer.Layer, pyglet.event.EventDispatcher):
    ...
    def on_key_press(self, k, m):
        if k == key.ESCAPE:
            ...
            if not self.show_interpreter:
                if not self.python_interpreter:
                    self.python_interpreter = cocos.scene.Scene(PythonInterpreterLayer())
                    self.python_interpreter.enable_handlers(True)
                self.python_interpreter.on_enter()
                self.show_interpreter = True
            else:
                self.python_interpreter.on_exit()
                self.show_interpreter = False
            return True
